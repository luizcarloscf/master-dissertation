...
try:
        ...
        for exp_folder in dirs:
            ...
            for camera in options.cameras:
                file_pattern = os.path.join(
                    sequence_folder, "c{camera_id:02d}s%08d.jpeg".format(camera_id=camera.id)
                )
                video_file = os.path.join(
                    options.folder,
                    "p{:03d}g{:02d}c{:02d}.mp4".format(person_id, gesture_id, camera.id),
                )
                if os.path.exists(video_file):
                    log.info("Video already created: '{}'", video_file)
                    continue
    ...
    log.info("remove video: {}", video_file)
