...
def draw_info_bar(
    image, text, x, y, background_color=(0, 0, 0), text_color=(255, 255, 255), draw_circle=False
):
...
while True:
    ...
    if len(images_data) == len(options.cameras):
        ...
        if n_sample % display_rate == 0:
            images = [
                cv2.imdecode(data, cv2.IMREAD_COLOR) for _, data in sorted(images_data.items())
            ]
            place_images(full_image, images)
            # display_image = cv2.resize(full_image, (0, 0), fx=0.5, fy=0.5)
            display_image = imutils.resize(full_image, width=1600)
