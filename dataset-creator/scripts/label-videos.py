...
for person_id, gestures in captures.items():
    for gesture_id, cameras in gestures.items():
        ...
        video_files = {
            camera: os.path.join(
                options.folder, "p{:03d}g{:02d}c{:02d}.mp4".format(person_id, gesture_id, camera)
            )
        ...
        labels_file = os.path.join(
            options.folder, "p{:03d}g{:02d}_spots.json".format(person_id, gesture_id)
        )
        ...
        put_text(
            full_image,
            "PERSON_ID: {:03d} GESTURE_ID: {:02d} ({:s})".format(
                person_id, gesture_id, gestures_labels[str(gesture_id)]
            ),
            x=20,
            y=0.8 * top_bar_h,
        )
        ...
        while True:
            ...
            if param.update_image:
                ...
                if frames is not None:
                    ...
                    draw_labels(
                        full_image, top_bar_h, labels, param.it_frames, param.n_loaded_frames
                    )
            ...
            if key == ord(keymap["previous_frames"]):
                param.it_frames -= keymap["big_step"]
                param.it_frames = (
                    param.n_loaded_frames - 1 if param.it_frames < 0 else param.it_frames
                )
                param.update_image = True
            ...
            if key == ord(keymap["begin_label"]):
                if labels[param.it_frames] == 0 and not waiting_end:
                    ...
                    labels[param.it_frames] == -1 or labels[param.it_frames] == 3
                ) and not waiting_end:
