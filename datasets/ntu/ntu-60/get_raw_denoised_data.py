...
missing_skes_logger1.info("{:^20}\t{}\t{}\t{}\t{}\t{}".format("Skeleton", "num_frames", "Actor1", "Actor2", "Start", "End"))
...
def denoising_by_length(ske_name, bodies_data):
    ...
    for bodyID, body_data in new_bodies_data.items():
        ...
        if length <= noise_len_thres:
            ...
            noise_len_logger.info("{}\t{}\t{:.6f}\t{:^6d}".format(ske_name, bodyID, body_data["motion"], length))
...
def remove_missing_frames(ske_name, joints, colors):
    ...
    if num_bodies == 2:  # DEBUG
        ...
        if max(cnt1, cnt2) > 0:
            if cnt1 > cnt2:
                info = "{}\t{:^10d}\t{:^6d}\t{:^6d}\t{:^5d}\t{:^3d}".format(ske_name, num_frames, cnt1, cnt2, start, end)
